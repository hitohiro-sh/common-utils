...
public class ObjectUtils {
	 ...
	 */
	static public <T,U> boolean fillObject_(
			T src, U dst, boolean throwException) 
					throws NoSuchMethodException,IllegalAccessException,InvocationTargetException {
		Class<?> srcClass = src.getClass();
		Class<?> dstClass = dst.getClass();
		...
		for (Method m : srcClass.getMethods()) {
			...
			if (name.startsWith("get")) {
				...
				String setMethodName = name.replaceFirst("^get", "set");
				// System.out.println(setMethodName);
				try {
					Method srcM = srcClass.getMethod(name);
					Method dstM = dstClass.getMethod(setMethodName, m.getReturnType());
